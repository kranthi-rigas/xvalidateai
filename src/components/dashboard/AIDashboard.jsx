...
export default function AIDashboard() {
    ...
    const highRiskList = useMemo(() =>
            dashboardAnalytics?.high_risk_tools || [],
        [dashboardAnalytics]
    );

    const allToolsList = useMemo(() =>
            dashboardAnalytics?.tool_kpis || [],
        [dashboardAnalytics]
    );
    ...
    const filteredTools = useMemo(() => {
        ...
        return dashboardAnalytics.tool_kpis.filter(
    ...
    const radarChartData = useMemo(() => ({
        ...
        datasets: [{
            label: 'Average Score',
            data: [
                pillarAverages?.privacy ?? 0,
                pillarAverages?.instructional ?? 0,
                pillarAverages?.usability ?? 0,
                pillarAverages?.dataQuality ?? 0
            ],
        ...
        plugins: {
            ...
            tooltip: {
                callbacks: {
                    afterLabel: (context) => {
                        const pillarMap = {
                            0: 'privacy_safety_score',
                            1: 'instructional_impact_score',
                            2: 'usability_score',
                ...
                callbacks: {
                    afterLabel: (context) => {
                        ...
                        const matchingTools = tools.filter(t =>
                            t.compliance_followed?.includes(complianceType)
                        );
    ...
    const recommendationChartData = useMemo(() => {
        const colors = {
            'Approved': '#27ae60',
            'Approved with limitations': '#3498db',
            'Restricted': '#f39c12',
            'Do not use': '#e74c3c',
        ...
        return {
            ...
            datasets: [{
                ...
                backgroundColor: data.map(d => colors[d.name] || '#95a5a6'),
                borderWidth: 2,
                borderColor: '#fff'
            }]
        };
    ...
    const privacyRiskScatterData = useMemo(() => {
        ...
        return {
            datasets: [{
                label: 'Tools',
                data: tools.map(tool => ({
                    x: tool.privacy_safety_score,
                    y: tool.overall_score,
    ...
    const privacyRiskScatterOptions = {
        ...
        scales: {
            x: {
                title: { display: true, text: 'Privacy & Safety Score →', font: { size: 14, weight: 'bold' } },
                beginAtZero: true,
                max: 100
            },
            y: {
                title: { display: true, text: 'Overall Compliance Score →', font: { size: 14, weight: 'bold' } },
                beginAtZero: true,
                max: 100
            }
        },
                ...
                callbacks: {
                    ...
                    label: (context) => {
                        ...
                        return [
                            '',
                            `🏢 ${tool.developer}`,
                            `🌐 ${tool.url || 'Not specified'}`,
                            '',
                            `📊 Overall: ${tool.overall_score.toFixed(0)}/100`,
                            `🔒 Privacy: ${tool.privacy_safety_score}/100`,
                            `📚 Instructional: ${tool.instructional_impact_score}/100`,
                            `⚙️ Usability: ${tool.usability_score}/100`,
                            `📈 Data Quality: ${tool.data_quality_score}/100`,
                            '',
                            `✅ ${tool.recommendation}`,
                            `🎓 ${tool.grade_level}`,
                            '',
                            `✓ Allowed:`,
                            tool.allowed_usage?.substring(0, 60) + '...',
                            '',
                            `✗ Restricted:`,
                            tool.restricted_usage?.substring(0, 60) + '...'
                        ];
                    }
                }
            }
    ...
    const overallScoreScatterData = useMemo(() => {
        ...
        tools.forEach(tool => {
            ...
            groupedData[rec].push({
                x: tool.overall_score,
                y: tool.instructional_impact_score,
                label: tool.tool_name
            });
        });
    ...
    const overallScoreScatterOptions = {
        ...
        scales: {
            x: {
                title: { display: true, text: 'Overall Score →', font: { size: 14, weight: 'bold' } },
                beginAtZero: true,
                max: 100
            },
            y: {
                title: { display: true, text: 'Instructional Impact →', font: { size: 14, weight: 'bold' } },
                beginAtZero: true,
                max: 100
            }
        },
    ...
    const renderHighRiskCell = (row, key) => {
        switch (key) {
            case "project_name":
                return <span style={{ fontWeight: 600, color: "#dc2626" }}>{row.project_name}</span>;
            case "developer":
                return row.developer || "—";
            case "overall_score":
                return (
                    <span style={{ fontWeight: 600, color: row.overall_score < 50 ? "#dc2626" : "#f59e0b" }}>
            {row.overall_score?.toFixed(0)}
          </span>
                );
            case "privacy_safety":
                return (
                    <span style={{ fontWeight: 600, color: row.privacy_safety < 50 ? "#dc2626" : "#f59e0b" }}>
            {row.privacy_safety}
          </span>
                );
            case "recommendation":
                return (
                    <span style={{
                        color: row.recommendation?.toLowerCase().includes("approved") ? "#059669" : "#dc2626",
                        fontWeight: 500
                    }}>
            {row.recommendation}
          </span>
            ...
            case "high_risk_reason":
                return <span style={{ fontSize: "0.9em", whiteSpace: "pre-wrap" }}>{row.high_risk_reason}</span>;
            default:
                return "—";
        }
    };
    ...
    const renderEvaluationCell = (row, key) => {
        switch (key) {
            case "tool_name":
                return <strong>{row.tool_name}</strong>;
            case "developer":
                return row.developer || "—";
            case "url":
                return row.url ? (
                    <a href={row.url} target="_blank" rel="noopener noreferrer" style={{ color: "#667eea" }}>
                        {row.url}
                    </a>
                ) : "—";
            case "overall_score":
            ...
            case "restricted_usage":
                return (
                    <span style={{ fontSize: "0.9em", maxWidth: "300px", display: "block", whiteSpace: "normal" }}>
            {row[key] || "Not specified"}
          </span>
                );
            case "grade_level":
                return row.grade_level || "Not Specified";
            default:
                return row[key] ?? "—";
        }
    };
    ...
    // ==================== LOADING STATE ====================
    if (pageLoading || !dashboardAnalytics) {
        ...
        <div className="dashboard__content">
            ...
            <div className="normal-container-styles">
                <div className="ai-cards-container">
                    <SingleScore title="Total Tools" value={dashboardAnalytics.overview.total_projects ?? 0} />
                    <SingleScore title="High-Risk Tools" value={dashboardAnalytics.overview.high_risk_count ?? 0} />
                    <SingleScore title="Approved Tools" value={dashboardAnalytics.overview.approved_count ?? 0} />
                    <SingleScore title="Rejected Tools" value={dashboardAnalytics.overview.rejected_count ?? 0} />
                </div>
                ...
                <div className="row y-gap-10">
                    <div className="col-md-3 col-sm-6">
                        <GaugeChart
                            ...
                            onClick={() => { setSelectedPillar("privacy_safety_score"); setIsModalOpen(true); }}
                        />
                    </div>
                    <div className="col-md-3 col-sm-6">
                        <GaugeChart
                            ...
                            onClick={() => { setSelectedPillar("instructional_impact_score"); setIsModalOpen(true); }}
                        />
                    </div>
                    <div className="col-md-3 col-sm-6">
                        <GaugeChart
                            ...
                            onClick={() => { setSelectedPillar("usability_score"); setIsModalOpen(true); }}
                        />
                    </div>
                    <div className="col-md-3 col-sm-6">
                        <GaugeChart
                            ...
                            onClick={() => { setSelectedPillar("data_quality_score"); setIsModalOpen(true); }}
                        />
                    </div>
                </div>
            </div>
            ...
            {isModalOpen && selectedPillar && (
                <Modal
                    title={`Tools - ${selectedPillar.replace(/_/g, " ")}`}
                    onClose={() => { setIsModalOpen(false); setSelectedPillar(null); }}
                >
            ...
            <div className="normal-container-styles">
                <div className="row">
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-3">
                        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                            <h3 style={{ fontSize: '1.3rem', marginBottom: '20px', color: '#2c3e50', borderBottom: '2px solid #ecf0f1', paddingBottom: '10px' }}>
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-3">
                        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                            ...
                            <div style={{ height: '300px' }}>
                                <Doughnut data={recommendationChartData} options={recommendationChartOptions} />
                            </div>
                        </div>
                    </div>

                    {/* Compliance Standards - Horizontal Bar */}
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-3">
                        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                            <h3 style={{ fontSize: '1.3rem', marginBottom: '20px', color: '#2c3e50', borderBottom: '2px solid #ecf0f1', paddingBottom: '10px' }}>
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-3">
                        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                            ...
                            <div style={{ height: '300px' }}>
                                <Doughnut data={gradeLevelChartData} options={gradeLevelChartOptions} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-3">
                        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                            ...
                            <div style={{ height: '300px' }}>
                                <Scatter data={privacyRiskScatterData} options={privacyRiskScatterOptions} />
                            </div>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem' }}>
                  <span style={{ width: '16px', height: '16px', borderRadius: '3px', background: '#27ae60' }}></span>
                  Low Risk (80+)
                </span>
                                <span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem' }}>
                  <span style={{ width: '16px', height: '16px', borderRadius: '3px', background: '#3498db' }}></span>
                  Medium (60-79)
                </span>
                                <span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem' }}>
                  <span style={{ width: '16px', height: '16px', borderRadius: '3px', background: '#f39c12' }}></span>
                  High (40-59)
                </span>
                                <span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem' }}>
                  <span style={{ width: '16px', height: '16px', borderRadius: '3px', background: '#e74c3c' }}></span>
                  Critical (&lt;40)
                </span>
                            </div>
                        </div>
            ...
            {/* HIGH-RISK TOOLS TABLE */}
                ...
                <div className="normal-container-styles" style={{ backgroundColor: "#fff3cd", borderLeft: "4px solid #f59e0b", padding: "20px" }}>
                    <h4 className="d-flex mb-20 justify-center" style={{ color: "#92400e" }}>
